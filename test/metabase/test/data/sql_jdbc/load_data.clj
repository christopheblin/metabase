...
(defn load-data-get-rows
  "Get a sequence of row maps for use in a `insert!` when loading table data. Used internally by `load-data-get-rows`."
...
(defn- make-insert! [driver conn {:keys [database-name], :as dbdef} {:keys [table-name], :as tabledef}]
  (let [escaped-table-name (apply hx/qualify-and-escape-dots (sql.tx/qualified-name-components driver database-name table-name))]
...
(def load-data-chunked!
  "Insert rows in chunks of 200 at a time."
...
(def load-data-add-ids!
  "Insert all rows at once; add IDs."
...
(def load-data-one-at-a-time-parallel!
  "Insert rows one at a time, in parallel."
